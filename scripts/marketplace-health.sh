...
#     (for vendored plugins).
#   - plugin.json name drifting from its marketplace.json entry name
#     (for vendored plugins).
#   - marketplace entries pointing at local paths that no longer exist
#     (rename left a stale path).
#   - dependencies referencing plugin names that aren't in the catalog
#     (typo or removed plugin).
#
# Reference-mode entries (source.url pointing outside this repo) are
# auto-detected and skipped from path/consistency checks — there's no
# local plugin.json to validate against.
#
...
for i, p in enumerate(plugins):
    ...
    # `dependencies` is a reserved field; must be a flat array of plugin name strings.
...
# ---------------------------------------------------------------------------
# 3. Plugin paths on disk (vendored only — reference entries skipped)
# ---------------------------------------------------------------------------
section "plugin paths on disk (vendored)"
...
for p in m.get("plugins", []):
    ...
    # Heuristic: if the local path exists, treat as vendored and validate.
    # If it doesn't exist, treat as reference (source.url points outside this repo) and skip.
    if not abs_path.is_dir():
        skipped_refs += 1
        continue
    checked += 1
    if not (abs_path / ".claude-plugin" / "plugin.json").is_file():
        print(f"  FAIL: {p['name']}: {src_path}/.claude-plugin/plugin.json missing (vendored entry without plugin.json)")
        errors += 1

if errors == 0:
    print(f"  PASS: {checked} vendored plugins validated; {skipped_refs} reference entries skipped")
sys.exit(1 if errors > 0 else 0)
PYEOF

# ---------------------------------------------------------------------------
# 4. plugin.json name + version match marketplace.json entry (vendored only)
# ---------------------------------------------------------------------------
section "plugin.json ↔ marketplace.json consistency (vendored)"
...
for entry in m.get("plugins", []):
    ...
    if not pj_path.is_file():
        continue  # reference entry — no local plugin.json
...
if errors == 0:
    print(f"  PASS: {checked} vendored plugins' plugin.json name + version match their marketplace entry")
sys.exit(1 if errors > 0 else 0)
PYEOF
